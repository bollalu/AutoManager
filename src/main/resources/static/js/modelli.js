...
{	
	...
	.done(function(data) {
		...
		$(data.modelli).each(function(index, element){
			var elimina = (idReferenziato("Modello",element.id) > 0)? 'Ref '+ idReferenziato("Modello",element.id):'<a href="/admin/modello/remove?id='+element.id+'" data-confirm="<table><tr><td>ID</td><td>' + element.id + '</td></tr>' +
